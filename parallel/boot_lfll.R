...
# lapply - future_lapply - lapply 

boot_lfll <- function(fut_globals) {
  handlers(global = TRUE)
  handlers("cli")
  plan(multisession, workers = 2)
  registerDoFuture()
  
  # create boots and bh cates inside future lapply because no global modification allowed (appending rows)
  ...
  lll_result <- lapply(names(ems), function(em) {
    ...
    boot1 <- future_lapply(quantiles, function(q) {
