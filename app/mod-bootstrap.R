...
bootstrap_server <- function(id, store) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$bootstrap, {
      ...
      progress <- AsyncProgress$new(value = 0, message = "Performing conditional bootstrap")
    ...
    observeEvent(input$stop_bootstrap, {
      if (running())
        interruptor$interrupt("Bootstrap canceled.")
    })
    ...
    # 
    #     incProgress(0, detail = HTML(paste("Performing bootstrap.")))
    # 
    #     # initialize counters and time storage vectors
    #     init <- numeric()
    #     end <- numeric()
